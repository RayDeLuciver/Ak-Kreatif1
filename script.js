...
function tampilkanGaleriDariStorage() {
  ...
  data.forEach((item, idx) => {
    ...
    div.className = `galeri-item ${item.kategori.replace(/\s+/g, '').toLowerCase()}`;
    div.innerHTML = `
...
function handleUpload(event) {
  ...
  const kategoriInput = document.getElementById("hashtagInput");
  ...
  const kategori = kategoriInput.value.trim() || "umum";
  ...
  const data = JSON.parse(localStorage.getItem("galeri-data")) || [];
  data.push({ url, nama, kategori, caption, tanggal });
...
function filterKategori(kategori) {
  const semuaItem = document.querySelectorAll(".galeri-item");

  semuaItem.forEach(item => {
    if (kategori === "semua") {
      item.style.display = "block";
    } else {
      const klasifikasi = item.className.split(" ");
      if (klasifikasi.includes(kategori.toLowerCase())) {
        item.style.display = "block";
      } else {
