...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Function to load and merge photos from public localStorage with better error handling
  function loadPhotosFromStorage() {
    console.log('Loading photos from public localStorage...');
    ...
    try {
      ...
      if (savedPhotos) {
        ...
        console.log('Public uploaded photos loaded:', uploadedPhotos.length);
  ...
  window.addEventListener('storage', function(e) {
    if (e.key === 'walkingStoryPhotos' || e.key === 'deletedDefaultPhotos') {
      console.log('Public storage changed, refreshing photos...');
      loadPhotosFromStorage();
      loadUploadedPhotosIntoCategories();
      loadUploadedPhotosIntoStreetPhotography();
      initializePhotoItems();
  ...
  // Function to dynamically load uploaded photos into category views with duplicate prevention
  function loadUploadedPhotosIntoCategories() {
    console.log('Loading uploaded photos into categories...');
    
    // Clear ALL existing uploaded photos first to prevent duplicates
    ...
    // Track processed photos to prevent duplicates
    const processedPhotoIds = new Set();
    
    uploadedPhotos.forEach(photo => {
      // Skip if already processed
      if (processedPhotoIds.has(photo.id)) {
        console.log('Skipping duplicate photo:', photo.title);
        return;
      }
      ...
      if (photo.category) {
        ...
        if (categoryView) {
          ...
          if (photoGrid) {
            // Check if photo already exists in this grid
            const existingPhoto = photoGrid.querySelector(`[data-photo-id="${photo.id}"]`);
            if (existingPhoto) {
              console.log('Photo already exists in grid, skipping:', photo.title);
              return;
            }
            ...
            photoItem.setAttribute('data-uploaded', 'true');
            photoItem.setAttribute('data-photo-id', photo.id); // Add unique identifier
            photoItem.innerHTML = `
            ...
            console.log('Added photo to category grid:', photo.title);
          }
        }
      }
    });
  ...
  // Function to load uploaded photos into Street Photography section with duplicate prevention
  function loadUploadedPhotosIntoStreetPhotography() {
    ...
    if (streetPhotographySection) {
      ...
      if (streetGrid) {
        ...
        // Track processed photos to prevent duplicates
        const processedStreetPhotoIds = new Set();
        
        uploadedPhotos.forEach((photo, index) => {
          // Skip if already processed
          if (processedStreetPhotoIds.has(photo.id)) {
            console.log('Skipping duplicate street photo:', photo.title);
            return;
          }
          
          processedStreetPhotoIds.add(photo.id);
          
          // Check if photo already exists in street grid
          const existingStreetPhoto = streetGrid.querySelector(`[data-photo-id="${photo.id}"]`);
          if (existingStreetPhoto) {
            console.log('Street photo already exists, skipping:', photo.title);
            return;
          }
          ...
          streetPhotoItem.setAttribute('data-uploaded', 'true');
          streetPhotoItem.setAttribute('data-photo-id', photo.id); // Add unique identifier
          ...
          console.log('Added photo to street photography grid:', photo.title);
        });
