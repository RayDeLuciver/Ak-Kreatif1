...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener('visibilitychange', function() {
    if (!document.hidden) {
      // Page became visible, refresh photos
      loadPhotosFromStorage();
      loadUploadedPhotosIntoCategories();
      loadUploadedPhotosIntoStreetPhotography();
    }
    ...
    if (streetPhotographySection) {
      ...
      if (streetGrid) {
        ...
        uploadedPhotos.forEach((photo, index) => {
            ...
            <div class="aspect-w-3 aspect-h-2 bg-gradient-to-br from-blue-400 to-blue-600 relative h-64">
              ...
              <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <div class="text-center text-white p-4">
                  ...
                  <p class="text-xs mt-1 bg-black bg-opacity-50 px-2 py-1 rounded">${photo.category || 'street-photography'}</p>
                </div>
              </div>
            </div>
          `;
          ...
          // Find placeholder boxes to replace
          const placeholderBoxes = streetGrid.querySelectorAll('.street-photo');
          console.log('Placeholder boxes found:', placeholderBoxes.length);
          
          if (placeholderBoxes.length > index) {
            // Replace the placeholder box at the current index
            console.log(`Replacing placeholder box ${index}`);
            streetGrid.replaceChild(streetPhotoItem, placeholderBoxes[index]);
          } else {
            // If no more placeholder boxes, add to the beginning
            console.log('Adding to beginning of grid');
